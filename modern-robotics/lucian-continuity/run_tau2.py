...
TAU2_SPEC = ROOT / "probes" / "tau2_provenance_conditions.json"
PROTOCOL_VERSION = "tau2-0.01-state-vs-provenance"


def load_spec() -> dict:
    with TAU2_SPEC.open("r", encoding="utf-8") as handle:
        return json.load(handle)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run tau state-vs-provenance continuity experiment against local Ollama."
    )
    ...
    parser.add_argument(
        "--condition",
        choices=["provenance_settled", "provenance_absent"],
        help="Optional single condition. Default runs both.",
    )
    ...
    condition_names = (
        [args.condition]
        if args.condition
        else ["provenance_settled", "provenance_absent"]
    )
    ...
    if args.out:
        ...
        output_path = RESULTS_DIR / f"{stamp}_{safe_model}_tau2_provenance.jsonl"
    ...
    for condition_name in condition_names:
        ...
        row = {
            ...
            "host": "ollama",
            "model": args.model,
            "provenance_condition": condition_name,
