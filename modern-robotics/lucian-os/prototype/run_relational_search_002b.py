"""Run RELATIONAL_SEARCH_002B against the local Ollama host.

Simulation only. No external actions are executed and no model weights are changed.
Run from modern-robotics/lucian-os:
    py prototype/run_relational_search_002b.py

RS-002B repairs the RS-002 format confound by giving every condition the same
interface-only LAND/HOLD scaffold. Target-status accuracy is the primary outcome.
...
CONDITIONS = (
    "format_only",
    "format_plus_principles",
    "format_plus_target_warrant_map",
    "format_plus_worked_target_warrant",
)
...
FORMAT_SCAFFOLD = """INTERFACE SCAFFOLD — FORMAT ONLY
...
def material(condition: str) -> str:
    if condition == "format_only":
        return "TRAINING MATERIAL: none beyond the shared interface scaffold. Use only the held-out facts."
    if condition == "format_plus_principles":
        return PRINCIPLES
    if condition == "format_plus_target_warrant_map":
        return WARRANT_MAP
    if condition == "format_plus_worked_target_warrant":
        return WORKED
...
# Exact held-out cases from the frozen RS-002 runner.
CASES = [
    case(
...
def expected_move_for_status(status: Any) -> str | None:
...
def main() -> None:
    ...
    output_path = RESULTS_DIR / f"relational_search_002b_{stamp}.jsonl"
    summary_path = RESULTS_DIR / f"relational_search_002b_{stamp}_summary.json"

    print("RELATIONAL_SEARCH_002B — Format-Controlled Target Warrant")
    print("Preregistered repair experiment; simulation only; no weights changed.")
    print("PRIMARY OUTCOME: target-status accuracy.")
    print(f"Model: {MODEL}")
    ...
    for i, c in enumerate(CASES):
        for condition in rotated_conditions(i):
            ...
            counts[condition]["move_token_valid"] += int(move_token_valid)
            ...
            event = {
                ...
                "experiment": "RELATIONAL_SEARCH_002B",
                "model": MODEL,
                "condition": condition,
                **c,
                "raw_output": raw,
    ...
    summary: dict[str, Any] = {
        ...
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "model": MODEL,
        "primary_outcome": "target_status_accuracy",
    ...
    for condition in CONDITIONS:
        c = counts[condition]
        pairs = sum(
            1 for values in family_status[condition].values()
        ...
        summary["conditions"][condition] = {
            ...
            "valid_schema": c["valid"],
            "move_token_valid": c["move_token_valid"],
    ...
    print("\nSUMMARY — PRIMARY SEMANTIC OUTCOME FIRST")
    for condition in CONDITIONS:
        s = summary["conditions"][condition]
        print(
            f"{condition:34s} status={s['status_correct_primary']}/12 "
            f"pairs={s['status_pairs_correct']}/6 over={s['overclaim']} "
            f"under={s['underclaim']} polarity={s['polarity_error']} | "
            f"valid={s['valid_schema']}/12 map={s['mapping_consistent']}/12 "
            f"joint={s['joint_correct']}/12"
        )

    print(f"\nRaw first-run output: {output_path}")
    print(f"Summary: {summary_path}")
    print("Do not rerun before inspecting the first complete run.")
