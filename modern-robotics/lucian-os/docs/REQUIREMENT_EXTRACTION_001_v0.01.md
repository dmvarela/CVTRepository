...
`experiments/requirement_extraction_001.json` contains 20 tasks covering:
...
2. Select an approved project substrate explicitly via `LUCIAN_MODEL`.
3. Do not expose the capability manifest to the host.
4. Run `run_requirement_extraction_v001.py`.
5. Preserve raw outputs.
6. Score with `evaluate_requirement_extraction_v001.py`.
...
The first live run should use the same frozen file without edits.
